...
Здесь нам не хватило дисциплины: мы добавили метод, не написав для него сначала тест. Это нормально, мы не всегда идеальны, но и закрывать на это глаза не стоит. Запуск `go test -cover` показал бы нам, что `String` не покрыт, что является хорошим поводом вернуться и спросить, стоит ли его тестировать ретроспективно. Не следует гнаться за 100% покрытием ради него самого, но в данном случае `String` имеет собственную логику (`fmt.Sprintf`), которую стоит закрепить тестом, поэтому давайте добавим тест.
...
Как видите,  ощибки могут быть преобразованы в строку с помощью метода `.Error()`, что мы и делаем для сравнения с нужной нам строкой. Мы также убеждаемся, что ошибка не `nil`, чтобы избежать вызова `.Error()` на `nil`.
...
Будет очень неприятно, если тест провалится, если кто-то захочет перефразировать сообщение об ошибке, и это просто слишком много деталей для нашего теста. Нам не _очень_ важна точная формулировка, просто при определённом условии возвращается какое-то осмысленное сообщение об ошибке, связанное с выводом средств.
...
## Добавление контекста с обёртыванием ошибок

Сравнение `err` с `ErrInsufficientFunds` здесь хорошо работает, потому что `Withdraw` — единственное, что может её произвести. Но в реальных программах есть слои — например, `Wallet` может использоваться `Bank`, который, в свою очередь, используется HTTP-обработчиком, и так далее. Если каждый слой просто возвращает полученную ошибку без изменений, то вызывающий код на нескольких уровнях выше видит только `insufficient funds`, не имея представления, какой счёт или какая операция на самом деле завершилась неудачей.
...
Глагол `%w` (введенный в Go 1.13) подобен `%v` тем, что подставляет строковое представление `err`, но он также делает то, чего не делает `%v`: он _обёртывает_ `err` внутри новой ошибки, которую возвращает `fmt.Errorf`, вместо того чтобы просто копировать его сообщение в новую, несвязанную строку. Попробуйте:
...
`errors.Is` работает, многократно вызывая [`errors.Unwrap`](https://pkg.go.dev/errors#Unwrap) на `err` (который знает, как извлечь обёрнутую ошибку обратно, потому что `fmt.Errorf` с `%w` производит значение с методом `Unwrap() error`), пока не найдёт совпадение или не закончатся ошибки для развёртывания. Вы снова увидите `errors.Is`, используемый именно по этой причине во вспомогательном методе `assertError` в главе [Отображения](maps.md), вместо `==` — это безопасный вариант по умолчанию, который также работает для ошибок, которые изначально не были обёрнуты.

Если вам нужно извлечь _типизированную_ ошибку (а не сравнивать со сторожевым значением, таким как `ErrInsufficientFunds`) из цепочки обёрнутых ошибок, для этого тоже есть эквивалентная функция: [`errors.As`](https://pkg.go.dev/errors#As). Глава [Типы ошибок](error-types.md) рассматривает это более подробно.
