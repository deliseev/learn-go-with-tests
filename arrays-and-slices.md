...
Мы уже сделали рефакторинг `Sum` — всё, что мы сделали, это заменили массивы срезами, поэтому никаких дополнительных изменений не требуется.
...
В этой реализации мы меньше беспокоимся о ёмкости. Мы начинаем с пустого среза `sums`
