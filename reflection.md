...
[Из X](https://x.com/peterbourgon/status/1011403901419937792?s=09)
...
Однако по замыслу вы не можете получить значения из отображения по индексу. Это делается только по _ключу_, что нарушает нашу абстракцию, черт возьми.
...
Мы отказались от проверки _порядка_ `got`, потому что отображения его не гарантируют, но это не значит, что мы должны отказаться от проверки всего, что касается его формы. Один только `assertContains` всё равно прошёл бы, если `walk` посетил элемент отображения дважды или пропустил один и проверил только оставшиеся элементы — до тех пор, пока конкретные значения, которые мы ищем, присутствуют где-то, сколько бы раз они ни встречались. В отличие от порядка, _длина_ `got` полностью предсказуема независимо от того, в каком порядке итерируется отображение, поэтому давайте проверим и это.
...
- Теперь, когда вы знаете о рефлексии, постарайтесь избегать ее использования.### Известное ограничение: циклические ссылки

Наш `walk` вызовет переполнение стека, если вы передадите ему структуру, которая содержит указатель на саму себя (или любой цикл указателей). Например:
...
Исправление этого остается в качестве упражнения. Стандартный подход заключается в отслеживании уже посещенных адресов указателей и их пропуске при повторном входе. Вам понадобится `map[uintptr]bool` — `uintptr` представляет собой необработанный числовой адрес указателя, который можно получить через `reflect.Value.Pointer()` для видов указателей. Поскольку отображение должно существовать на протяжении всего обхода, вам понадобится внутренняя вспомогательная функция, которая будет передавать его в качестве параметра, при этом публичная функция `walk` будет создавать отображение и вызывать эту вспомогательную функцию.
