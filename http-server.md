...
Однако мы всё ещё не должны быть довольны. Кажется неправильным, что наш сервер знает счеты.
...
Обратите внимание, что мы всё ещё не беспокоимся о создании хранилищ *пока что*, мы просто хотим, чтобы компилятор проходил как можно скорее.
...
Попробуйте запустить тесты, и мы должны вернуться к компилируемому коду — но тест всё ещё падает.
...
*Это разрешено!* У нас всё ещё есть тест, проверяющий, что все должно работать правильно, но он не касается конкретного модуля, с которым мы работаем (`InMemoryPlayerStore`).
